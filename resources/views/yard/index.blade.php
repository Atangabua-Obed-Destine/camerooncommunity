    ...
    <div class="yard-container" x-data="yardApp()" @room-selected.window="onRoomSelected($event.detail)" @yard-back.window="goBack()" @yard-open-new-chat.window="openNewChat()" @toggle-room-info.window="toggleInfo()" @open-room-info.window="openInfo($event.detail?.roomId)" @room-type-changed.window="activeRoomType = $event.detail.roomType"
        ...
        <div class="yard-panel yard-panel--list"
            ...
            <header class="yard-header">
                ...
                <div class="flex items-center gap-2 w-full" x-show="!newGroupStep">
                    ...
                    <div class="flex items-center gap-1">
                        ...
                        <div class="relative" x-data="{ moreOpen: false }">
                            <button type="button" @click="moreOpen = !moreOpen" class="yard-header__btn relative"
                            ...
                            <div x-show="moreOpen" @click.away="moreOpen = false" x-transition x-cloak
                                ...
                                <button type="button"
                                        @click="moreOpen = false; Livewire.dispatch('open-connections', { tab: 'requests' })"
                                        class="w-full flex items-center gap-3 px-4 py-2.5 text-sm text-slate-700 hover:bg-slate-50 transition-colors text-left">
                                    ...
                                    <span class="flex-1" x-text="$store.lang.t('Contacts', 'Contacts')"></span>
                                    @auth
                                        <livewire:yard.connections-badge :variant="'pill'" />
                                    @endauth
                                </button>
                                <button type="button"
                                        @click="moreOpen = false; Livewire.dispatch('open-communities')"
                                        class="w-full flex items-center gap-3 px-4 py-2.5 text-sm text-slate-700 hover:bg-slate-50 transition-colors text-left">
                ...
                <div class="flex items-center gap-3 w-full" x-show="newGroupStep === 1" x-cloak>
                    ...
                    <div class="flex-1">
                        <h2 class="text-base font-semibold text-slate-900" x-text="$store.lang.t('Add group members', 'Ajouter des membres')"></h2>
                        <p class="text-xs text-slate-500" x-text="selectedMembers.length + ' ' + $store.lang.t('selected', 'sélectionnés')"></p>
                    </div>
                </div>
                ...
                <div class="flex items-center gap-3 w-full" x-show="newGroupStep === 2" x-cloak>
                    ...
                    <h2 class="text-base font-semibold text-slate-900" x-text="$store.lang.t('New group', 'Nouveau groupe')"></h2>
                </div>
            </header>
        ...
        <aside class="yard-drawer" :class="{ 'yard-drawer--open': menuOpen }" x-cloak>
            ...
            <nav class="yard-drawer__nav">
                ...
                <a href="{{ auth()->user()?->profileUrl() ?? route('profile') }}" class="yard-drawer__item">
