...
@php
    ...
    $iconActive    = 'text-cm-yellow drop-shadow-[0_2px_4px_rgba(0,0,0,0.55)] scale-110';
...
{{-- THE YARD / MESSENGER (labeled "GoConnect") --}}
<a href="{{ route('yard') }}"
   class="{{ $tabBase }} group transition-colors {{ $isYard ? $activeWrapper : 'hover:bg-white/5' }}"
   :title="$store.lang.t('GoConnect', 'GoConnect')"
   aria-label="GoConnect">
...
@foreach($soonTabs as $tab)
    <button type="button"
        ...
        @switch($tab['key'])
            @case('easygoparcel')
                <svg class="{{ $iconSize }} {{ $iconSoon }}" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.4">
